...
import { editJob } from '@/lib/api/jobs';
...
const JobsEditPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setIsLoading(true);
      await editJob(
        job as JobCreateRequest,
        fileToUpload,
        jobId,
        registerData.imageUrl.length == 0,
      );
  ...
  return (
    <>
      <Header title={'게시글 수정'} showBackButton={true} centerTitle={false} />
      <div className="flex w-full flex-col items-center gap-5 px-6 py-3">
        ...
        <div className="flex w-full flex-col gap-3 text-2xl font-light">
          ...
          <InputWithIcon
            ...
            onClick={() => router.push(`/jobs/${jobId}/edit/location`)}
            readOnly
          />
        </div>
        <Button
